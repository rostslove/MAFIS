...
def profile_m4_artifact(
    path: str | Path,
    target_column: str = M4_TARGET_COLUMN,
    task_type: str = M4_TASK_TYPE,
    sample_rows: int = 1000,
    sample_columns: int = 200,
) -> Dict[str, Any]:
    X, y, metadata = load_m4_artifact_data(path, mmap_mode="r")
    if target_column != metadata.get("target_column", M4_TARGET_COLUMN):
        raise ValueError(f"Target column '{target_column}' is not in M4 artifact.")

    n_samples = int(metadata.get("n_samples") or X.shape[0])
    n_features = int(metadata.get("n_features") or X.shape[1])
    row_count = min(n_samples, sample_rows)
    col_count = min(n_features, sample_columns)
    row_idx = np.linspace(0, n_samples - 1, row_count, dtype=int) if row_count else np.asarray([], dtype=int)
    col_idx = np.linspace(0, n_features - 1, col_count, dtype=int) if col_count else np.asarray([], dtype=int)
    sample = np.asarray(X[np.ix_(row_idx, col_idx)], dtype=np.float32) if row_count and col_count else np.empty((0, 0))

    feature_names = [f"f_{idx}" for idx in range(min(n_features, 20))]
    numeric_names = [f"f_{idx}" for idx in range(min(n_features, 30))]
    class_balance = metadata.get("class_balance") or {
        str(value): int(count)
        for value, count in zip(*np.unique(np.asarray(y).astype(str), return_counts=True))
    }
    counts = np.asarray(list(class_balance.values()), dtype=float)
    imbalance_ratio = float(counts.min() / counts.max()) if counts.size and counts.max() else 1.0

    feature_stats: Dict[str, Any] = {
        "numeric_count": n_features,
        "sampled_rows": int(row_count),
        "sampled_columns": int(col_count),
        "has_missing": bool(np.isnan(sample).any()) if sample.size else False,
        "missing_ratio": round(float(np.isnan(sample).mean()), 4) if sample.size else 0.0,
    }
    if sample.size:
        mean_values = np.nanmean(sample, axis=0)
        std_values = np.nanstd(sample, axis=0)
        mean_of_means = float(np.nanmean(np.abs(mean_values))) if mean_values.size else 0.0
        feature_stats["mean_std_ratio"] = round(
            float(np.nanmean(std_values) / mean_of_means) if mean_of_means else 0.0,
            3,
        )
        feature_stats["sample_mean_abs"] = round(float(np.nanmean(np.abs(mean_values))), 4)
        feature_stats["sample_std_mean"] = round(float(np.nanmean(std_values)), 4)

    issues = []
    if n_samples > 50000:
        issues.append("large_dataset")
    if n_features > 1000:
        issues.append("long_time_series_window")
    if imbalance_ratio < 0.7:
        issues.append(f"class_imbalance (ratio={imbalance_ratio:.3f})")
    ...
    return {
        ...
        "target_stats": {
            "unique_values": len(class_balance),
            "type": task_type,
            "distribution": class_balance,
        },
        "is_imbalanced": imbalance_ratio < 0.7,
        "imbalance_ratio": round(imbalance_ratio, 3),
        "is_time_series": True,
        "window_length": int(metadata.get("window_length") or n_features),
        "window_length_mode": metadata.get("window_length_mode", "fixed"),
        "storage_format": metadata.get("storage_format", M4_ARTIFACT_KIND),
